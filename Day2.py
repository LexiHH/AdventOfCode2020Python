...
string_formatted = string_unformatted.split('\n')
min_numbers = []
max_numbers = []
required_letters = []
passwords = []
...
def get_min_numbers(entire_strings):
    for string in entire_strings:
        min_number = re.search(".*?(?=-)", string).group(0)
        min_numbers.append(min_number)


def get_max_numbers(entire_strings):
    for string in entire_strings:
        max_number = re.search("(?<=-)(.*?)(?=\ )", string).group(0)
        max_numbers.append(max_number)


def get_required_letters(entire_strings):
    for string in entire_strings:
        required_letter = re.search("(?<=\ )(.*?)(?=:)", string).group(0)
        required_letters.append(required_letter)


def get_passwords(entire_strings):
    for string in entire_strings:
        password = re.search("(?<=:\ )(.*)", string).group(0)
        passwords.append(password)


def create_sub_strings(entire_strings):
    get_min_numbers(entire_strings)
    get_max_numbers(entire_strings)
    get_required_letters(entire_strings)
    get_passwords(entire_strings)


def analyse_passwords_day_one(entire_strings):
